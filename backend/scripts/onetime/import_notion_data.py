"""One-time Notion -> Postgres import.

Pulled the lab's existing Notion inventory database into the
Container/Chemical/Location/Ingredient/WeightReading tables ahead of the
Location.barcode backfill (see the "Backfill Location.barcode with LOC-<id>
format" commit). Already run against production data; kept here for
reference, not wired up as a `manage.py` command since it isn't meant to
run again as-is (the `location_map` below is a snapshot of that one Notion
database's location IDs, and would need to be redone for any future import).

Needs NOTION_SECRET and NOTION_DB_ID in the environment. Run with
`poetry run python scripts/onetime/import_notion_data.py` from `backend/`.
"""
...
# Standalone script living outside apps/ — configure Django manually before
# importing anything that touches the ORM.
